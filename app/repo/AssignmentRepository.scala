...
class AssignmentRepository  @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  def insertAssignment(assignments: Assignments): Future[Int] = {
    ...
    val insertAction = assignmentTable.returning(assignmentTable.map(_.id)) += assignments
    db.run(insertAction)
  }
  ...
  def updateAssignment(assignments: Assignments): Future[Int] = {
    ...
    val updateCertificate = for {assignment <- assignmentTable if assignment.id === assignments.id} yield assignment
  ...
  def geAssignmentByUser(id: Int): Future[List[Assignments]] = {
    ...
    val getCertificate = for {assignment <- assignmentTable if assignment.userId === id} yield assignment

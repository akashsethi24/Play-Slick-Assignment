...
class LanguageRepository  @Inject()(protected val dbConfigProvider: DatabaseConfigProvider)
  ...
  def updateLanguage(languages: Languages): Future[Int] = {
    ...
    val updateCertificate = for {language <- languageTable if language.id === languages.id} yield language
  ...
  def getLanguageByUser(id: Int): Future[List[Languages]] = {
    ...
    val getCertificate = for {language <- languageTable if language.userId === id} yield language

...
trait UserTable {
  ...
  class UserTable(tag: Tag) extends Table[User](tag, "student") {
    ...
    val mobile = column[BigInt]("user_mobile", O.SqlType("BIGINT(10)"))
    val isAdmin = column[Boolean]("user_isAdmin", O.SqlType("BOOLEAN"))

    def * = (id, name, email, password,mobile,isAdmin) <> (User.tupled,User.unapply)
...
trait UserRepository extends UserTable {
  ...
  def insert(user:User):Future[Int] = {
    ...
    val insertUser = userTable.returning(userTable.map(_.id)) += user
    db.run(insertUser)
